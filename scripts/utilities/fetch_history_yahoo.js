...
// Uses the same robust loading pattern as scrape_yahoo.js
...
async function fetchDividends(symbol) {
  ...
  try {
    ...
    if (!yahoo) {
      console.warn(`yahoo-finance2 module not loaded - cannot fetch dividends for ${symbol}`);
      return null;
    }

    // Preferred: use chart() with events='div' - this is the v3.x recommended approach
    if (typeof yahoo.chart === 'function') {
      debugLog(`Trying yahoo.chart for ${symbol} with events=div`);
      try {
        // chart() options: period1/period2 as Date or 'YYYY-MM-DD' string, or use period='max'
        const chartResult = await yahooCallWithRetries(yahoo, 'chart', [symbol, { 
        ...
        debugLog(`chart() returned for ${symbol}:`, chartResult ? 'data' : 'null');
        ...
        if (chartResult && chartResult.events && chartResult.events.dividends) {
          ...
          debugLog(`Found ${Object.keys(dividends).length} dividend events for ${symbol}`);
          ...
          if (out.length) {
            debugLog(`Returning ${out.length} normalized dividend records for ${symbol}`);
            return out;
          }
        } else {
          debugLog(`No dividend events in chart response for ${symbol}`);
        }
      } catch (err) {
        console.warn(`yahoo-finance2.chart failed for ${symbol}: ${err && err.message}`);
      }
    } else {
      debugLog(`yahoo.chart not available for ${symbol}`);
    }

    // Fallback: try historical() which internally maps to chart() in v3.x
    if (typeof yahoo.historical === 'function') {
      debugLog(`Trying yahoo.historical for ${symbol}`);
      try {
        const hist = await yahooCallWithRetries(yahoo, 'historical', [symbol, { 
          period1: '2000-01-01',
          period2: new Date().toISOString().slice(0, 10),
        ...
        if (Array.isArray(hist) && hist.length) {
          debugLog(`historical() returned ${hist.length} records for ${symbol}`);
          const out = hist
          ...
          if (out.length) {
            debugLog(`Returning ${out.length} dividend records from historical() for ${symbol}`);
            return out;
          }
        }
      } catch (err) {
        console.warn(`yahoo-finance2.historical dividends fetch failed for ${symbol}: ${err && err.message}`);
      }
    } else {
      debugLog(`yahoo.historical not available for ${symbol}`);
    }

  } catch (err) {
    console.warn(`yahoo-finance2 failed for ${symbol}: ${err && err.message}`);
  }
...
async function fetchQuoteSummaryModules(symbol, modules = []) {
  // Prefer the yahoo-finance2 library's quoteSummary method when available
  try {
    const yahoo = await ensureYahoo();
    if (yahoo && typeof yahoo.quoteSummary === 'function') {
      debugLog(`Trying yahoo.quoteSummary for ${symbol} with modules:`, modules);
      try {
        const result = await yahooCallWithRetries(yahoo, 'quoteSummary', [symbol, { modules }]);
        debugLog(`quoteSummary returned for ${symbol}:`, result ? 'data' : 'null');
        return result;
      } catch (err) {
        console.warn(`yahoo-finance2.quoteSummary failed for ${symbol}: ${err && err.message}`);
      }
    } else {
  ...
  const url = `https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encodeURIComponent(symbol)}?modules=${encodeURIComponent(modParam)}`;
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      if (res.status === 429) {
        ...
        console.warn(`Yahoo quoteSummary 429 for ${symbol}, waiting ${wait}ms then retrying`);
        await sleep(wait);
        continue;
      }
      if (!res.ok) {
        console.warn(`Yahoo quoteSummary fetch fail for ${symbol} status=${res.status}`);
        return null;
      }
      const json = await res.json();
      return json;
    } catch (err) {
      console.warn(`Error fetching quoteSummary for ${symbol}: ${err.message}`);
...
async function upsertDividend(connection, symbol, exDate, amount, status = 'confirmed') {
  ...
  `;
  try {
    await connection.execute(sql, [symbol, exdStr, amount, status]);
  } catch (err) {
    console.warn(`Failed upsert dividend ${symbol} ${exdStr} ${amount}: ${err.message}`);
  }
}

async function upsertEarning(connection, symbol, earningsDate, epsActual, epsEstimate, revenueActual, fiscalQuarter, fiscalYear, isEstimate) {
  const ed = parseDate(earningsDate);
  if (!ed) return;
  const edStr = ed.toISOString().slice(0,19).replace('T',' ');
  const sql = `
    INSERT INTO securities_earnings (
      symbol, earnings_date, earnings_date_end, is_estimate, eps_actual, eps_estimate, revenue_actual, fiscal_quarter, fiscal_year, data_source
    ) VALUES (?, ?, NULL, ?, ?, ?, ?, ?, ?, 'yahoo')
    ON DUPLICATE KEY UPDATE
  ...
  `;
  try {
    await connection.execute(sql, [symbol, edStr, !!isEstimate, epsActual, epsEstimate, revenueActual, fiscalQuarter, fiscalYear]);
  } catch (err) {
    console.warn(`Failed upsert earnings ${symbol} ${edStr} eps:${epsActual} est:${epsEstimate} ${err.message}`);
  }
}
...
async function computeAndWriteTTM(connection, symbol) {
  ...
  try {
    ...
    console.warn(`Failed update metadata TTM for ${symbol}: ${err.message}`);
  }
}

async function processSymbol(connection, symbol) {
  console.log(`Fetching history for ${symbol}...`);

  // Dividends - now returns structured array directly (no CSV parsing needed)
  const dividendRows = await fetchDividends(symbol);
  if (dividendRows && dividendRows.length) {
    console.log(`  found ${dividendRows.length} dividend rows`);
    for (const r of dividendRows) {
      await upsertDividend(connection, symbol, r.ex_dividend_date, r.dividend_amount, 'confirmed');
    }
  } else {
    console.log(`  no dividend data available from yahoo-finance2 for ${symbol}`);
  }

  // Earnings: request earningsHistory and incomeStatementHistory
  const modules = ['earningsHistory','incomeStatementHistory','earnings'];
  const resp = await fetchQuoteSummaryModules(symbol, modules);
  ...
  debugLog(`quoteSummary payload for ${symbol}:`, payload ? Object.keys(payload) : 'null');
    ...
    if (payload.earningsHistory && Array.isArray(payload.earningsHistory.history)) {
      ...
      for (const item of hist) {
        ...
        if (earningsDate && !isNaN(earningsDate.getTime()) && (epsActual !== null || epsEstimate !== null)) {
          await upsertEarning(connection, symbol, earningsDate, epsActual, epsEstimate, null, fiscalQuarter, fiscalYear, epsActual === null);
        }
      }
    } else {
    ...
    if (payload.incomeStatementHistory && Array.isArray(payload.incomeStatementHistory.incomeStatementHistory)) {
      ...
      for (const q of ins) {
        ...
        if (earningsDate && epsActual !== null) {
          await upsertEarning(connection, symbol, earningsDate, epsActual, null, revenueActual, null, (earningsDate ? earningsDate.getFullYear(): null), false);
        }
      }
    }
  } else {
    console.log(`  no quoteSummary payload for ${symbol}`);
  }

  // Compute TTM and write to metadata
  await computeAndWriteTTM(connection, symbol);
}
...
async function main() {
  ...
  try {
    ...
    else if (allFlag) symbols = await getUniqueSymbols(connection, true, false);
    else if (allMetadataFlag) symbols = await getUniqueSymbols(connection, false, true);
    else { console.log('Usage: node scripts/fetch_history_yahoo.js --symbol AAPL | --all | --all-metadata'); process.exit(1); }

    console.log(`Processing ${symbols.length} symbols...`);
    for (let i=0;i<symbols.length;i++) {
      const s = symbols[i];
      console.log(`\n[${i+1}/${symbols.length}] ${s}`);
      try { await processSymbol(connection, s); } catch (err) { console.warn(`Error processing ${s}: ${err.message}`); }
      // gentle pacing
      if (i < symbols.length-1) await sleep(700);
    }
