...
async function scrapeWSJ(browser, security, outputDir) {
  ...
  try {
    ...
    let regular_price = cleanNumberText($('[data-field="Last"] .WSJTheme--last--3M1ny').first().text().trim() || $('[data-field="Last"] [class*="last"]').first().text().trim());
    ...
    if (!regular_price) {
      regular_price = cleanNumberText($('span:contains("Last")').parent().find('span').last().text().trim());
    }
