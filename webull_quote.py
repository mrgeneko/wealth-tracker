        ...
        tell document "{numbers_file}"
            tell sheet "{sheet_investments}"
                tell table "{table_investments}"
                    ...
                    set ticker_col to 0
                    repeat with i from 1 to column count
                        if value of cell i of row 1 is "webull_ticker" then
                            set ticker_col to i
                            exit repeat
                        end if
                    end repeat
                    if ticker_col is 0 then error "Ticker column not found."

                    set rowCount to row count
                    repeat with r from 2 to rowCount
                        set tickerVal to value of cell ticker_col of row r
                        if tickerVal is not missing value and tickerVal is not "" then
                            {chr(10).join([
                                f'if tickerVal is "{data["webull_ticker"]}" then set value of cell price_col of row r to "{data["last_price"]}"'
