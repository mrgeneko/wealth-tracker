...
describeDb('TTM Migration Integration Test', () => {
  let connection;
  ...
  test('TTM calculation with dividends, earnings, and stock splits', async () => {
    const sym = 'TTMTEST';
    ...
    await connection.execute('DELETE FROM securities_dividends_backup WHERE ticker = ?', [sym]).catch(() => {});
    ...
    // Insert metadata row
    await connection.execute(`INSERT INTO securities_metadata (ticker, quote_type, short_name, currency) VALUES (?, 'EQUITY', 'TTM Test Corp', 'USD')`, [sym]);
    ...
    await connection.execute(`INSERT INTO securities_dividends (ticker, ex_dividend_date, dividend_amount, status, data_source) VALUES (?, ?, ?, 'paid', 'test')`, [sym, exMid, 2.0]);
    // Another dividend amount should be counted as-is
    await connection.execute(`INSERT INTO securities_dividends (ticker, ex_dividend_date, dividend_amount, status, data_source) VALUES (?, ?, ?, 'paid', 'test')`, [sym, exRecent, 0.5]);
    // Old dividend outside window
    await connection.execute(`INSERT INTO securities_dividends (ticker, ex_dividend_date, dividend_amount, status, data_source) VALUES (?, ?, ?, 'paid', 'test')`, [sym, exOld, 5.0]);
    ...
    for (let i = 0; i < epsDates.length; i++) {
      ...
      await connection.execute(`INSERT INTO securities_earnings (ticker, earnings_date, is_estimate, eps_actual, fiscal_quarter, fiscal_year, data_source) VALUES (?, ?, FALSE, ?, 'Q${i+1}', 2025, 'test')`, [sym, d + ' 00:00:00', epsVals[i]]);
    }
    ...
    // Verify results
    const [metaRows] = await connection.execute('SELECT ttm_dividend_amount, ttm_eps, ttm_last_calculated_at FROM securities_metadata WHERE ticker = ?', [sym]);
    ...
    expect(Math.abs(ttmDiv - 1.5)).toBeLessThan(0.0001);

    // ttm eps should be latest 4 eps values (0.5 + 0.7 + 0.8 + 0.9 = 2.9)
    expect(Math.abs(ttmEps - 2.9)).toBeLessThan(0.0001);
